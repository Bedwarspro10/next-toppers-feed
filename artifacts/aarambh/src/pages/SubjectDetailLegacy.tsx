...
async function loadFolders(subject: string, parentId: string | null): Promise<Folder[]> {
  ...
  if (!snap) return [];
  const all = snap.docs.map((d) => ({ id: d.id, ...d.data() } as Folder));
  if (parentId === null) {
    return all.filter((f) => !f.parentFolderId || f.parentFolderId === "")
              .sort((a, b) => (a.order ?? 999) - (b.order ?? 999));
  }
  return all.filter((f) => f.parentFolderId === parentId)
            .sort((a, b) => (a.order ?? 999) - (b.order ?? 999));
}
...
async function loadGeneralResources(subject: string): Promise<Resource[]> {
  ...
  const all   = [...files, ...lects].filter((r) =>
...
export default function SubjectDetail() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const lects = snap.docs
        .map((d) => ({ id: d.id, ...d.data() } as LectureDoc))
        .sort((a, b) => (a.order ?? 999) - (b.order ?? 999));
      setPremiumLects(lects);
    }, () => setPremiumLects([]));
    return unsub;
  }, [subject]);
